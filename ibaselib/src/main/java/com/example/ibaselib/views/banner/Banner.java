package com.example.ibaselib.views.banner;
 ...
 *            .start();
 *     // 需要根据实际情况 选择 BannerConfig 中的几种 Type;
 *     banner.updateBannerStyle( BannerConfig.NUM_INDICATOR_TITLE);
 *
 *     // 如果遇上类似SwipeRefreshLayout 刷新的 时候 ，需要更新数据
 *     banner.update(list);
 *
 *     // 可以设置  指示器的位置
 *     banner.   banner.setIndicatorGravity(BannerConfig.xxx);
 *
 *     // 可以在xml 文件中 设置指示器的样式  或者 title 的样式
 *        也可以在xml 文件中 引入自定义布局；
 *
 *      // 也可以添加动画
 *      banner.setBannerAnimation(xxx)
 */
